...
fun ContextMenuDialog(
    ...
    if (showDeleteConfirmDialog) {
        AlertDialog(
            ...
            text = { Text("Apakah Anda yakin ingin menghapus file ini via SFTP? Tindakan ini tidak dapat dibatalkan.") },
            confirmButton = {
                TextButton(onClick = {
    ...
    if (showAdminRequiredDialog) {
        AlertDialog(
            onDismissRequest = { showAdminRequiredDialog = false },
            icon = { Icon(Icons.Default.Lock, contentDescription = null, tint = MaterialTheme.colorScheme.error) },
            title = { Text("Mode Admin Nonaktif") },
            text = { Text("Fitur hapus file terkunci. Untuk menghapus file via SFTP, silakan aktifkan 'Mode Admin SFTP' pada menu Pengaturan.") },
            confirmButton = {
                TextButton(onClick = { showAdminRequiredDialog = false }) {
                    Text("Mengerti")
                }
            }
        )
            ...
            Column(modifier = Modifier.fillMaxWidth()) {
                ...
                if (adminModeSftp) {
                    ...
                    ContextMenuItem(
                        ...
                        label = "Hapus File (Terkunci - Mode Admin OFF)",
                        tint = Color.Gray,
                        onClick = {
